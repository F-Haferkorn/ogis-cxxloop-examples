...
void square(short noRows, short noColumns) {
  loop(noRows, newline()) // use loop() with/without post-operations
      loop(noColumns) star();
}
...
void triangular(short noRows) {
  named_loop_up(row, noRows,
                newline()) // using named_loop_up() with post operation.
      loop(row + 1) star();
}
...
void centered_triangular_upside(short nofRows) {
  named_loop_up(row, nofRows,
                newline()) // using named_loop_down() with a post operations
  {
