...
#define CPPMACRO_NTIMES_UP(type, indexVarName, nbrOfRepetitions, ...)          \
  for (type indexVarName = 0; indexVarName < nbrOfRepetitions;                 \
       indexVarName++, ##__VA_ARGS__)

///////////////////////////////////////////
#define typed_loop(type, nbrOfRepetitions, ...)                                \
  CPPMACRO_NTIMES_UP(type, CPPMACRO_UNIQUE_ID(), nbrOfRepetitions,             \
                     ##__VA_ARGS__)
...
#define named_loop_up(indexVarName, nbrOfRepetitions, ...)                     \
  typed_named_loop_up(decltype(nbrOfRepetitions), indexVarName,                \
                      nbrOfRepetitions, ##__VA_ARGS__)
#endif

typedef unsigned char Byte;
void matrix_copy_w_stride(double *tgt, double *src, Byte nbofRows,
...
void matrix_add_using_While(double *tgt, double *src, Byte nbofRows,
                            Byte nbofColumns) {
  while (nbofRows--)
    while (nbofColumns--)
      *tgt++ += *src++;
